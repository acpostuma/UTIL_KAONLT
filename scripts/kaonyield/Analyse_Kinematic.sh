...
RunListFile="${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}"
...
if [ -f "${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}_MissingAnalyses" ]; then
    rm "${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}_MissingAnalyses"
else touch "${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}_MissingAnalyses"
fi
...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	echo "Analysis not found for run $runNum in ${UTILPATH}/scripts/kaonyield/OUTPUT/"
	echo "${runNum}" >> "${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}_MissingAnalyses"
...
if [ $TestingVar == 1 ]; then
    ...
    cp "${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}_MissingAnalyses" "$REPLAYPATH/UTIL_BATCH/InputRunLists/${KINEMATIC}_MissingAnalyses"
    if [ $Autosub == 1 ]; then
	while IFS='' read -r line || [[ -n "$line" ]]; do
	    runNum=$line
	    if [ -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/${runNum}_-1_Analysed_Data.root" ]; then
	...
	done < "${UTILPATH}/scripts/kaonyield/Kinematics/${KINEMATIC}_MissingAnalyses"
	...
	if [[ $prompt2 == "y" || $prompt2 == "Y" || $prompt2 == "yes" || $prompt2 == "Yes" ]]; then
		...
		runNum=$line
		if [ ! -f "${UTILPATH}/OUTPUT/Analysis/KaonLT/${runNum}_-1_Analysed_Data.root" ]; then
		    python3 $UTILPATH/scripts/kaonyield/src/Kaonyield.py "Kaon_coin_replay_production" ${runNum} "-1"
		fi
	    done < "$RunListFile"
...
if [ $TestingVar == 1 ]; then
	...
	root -b -l -q "${UTILPATH}/scripts/kaonyield/PlotKaonPhysics.C(\"${KINFILE}\", \"${KINEMATIC}_Kaons\")"
